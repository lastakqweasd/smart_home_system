            ...
            <div class="control-group">
              ...
              <div class="toggle-switch">
                <input 
                  type="checkbox" 
                  :id="'power-' + device.id" 
                  v-model="deviceStates[device.id].power"
                >
                <label :for="'power-' + device.id"></label>
              </div>
            </div>
            
            <div v-if="device.type === '智能灯光'" class="control-group">
              <label>亮度</label>
              <input 
                type="range" 
                min="0" 
                ...
                :disabled="!deviceStates[device.id].power"
              >
              <span>{{ deviceStates[device.id].brightness }}%</span>
            </div>
            
            <div v-if="device.type === '空调'" class="control-group">
              <label>温度</label>
              <input 
                type="range" 
                min="16" 
    ...
    const initializeDeviceStates = () => {
      devices.value.forEach(device => {
        deviceStates.value[device.id] = {
          ...
          brightness: device.type === '智能灯光' ? device.status.brightness : 50,
          temperature: device.type === '空调' ? device.status.temperature : 22
        }
      })
    }
      ...
      Object.keys(deviceStates.value).forEach(deviceId => {
        if (deviceStates.value[deviceId].selected) {
          ...
          selectedDevices.push({
            deviceId,
            status: {
              on: deviceStates.value[deviceId].power,
              ...(device.type === '智能灯光' && { brightness: deviceStates.value[deviceId].brightness }),
              ...(device.type === '空调' && { temperature: deviceStates.value[deviceId].temperature })
            }
          })
        }
      })
